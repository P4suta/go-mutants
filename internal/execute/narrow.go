...
// binary the run does not start, or one that names no test. Both are refused
// through the caller's own error, because the two callers — a mutant run and a
// control — refuse under different codes and have to say so about different
// things; the rule is the same and lives once.
