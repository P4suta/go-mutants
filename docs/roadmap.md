...
| # | What | Done when |
| ---: | --- | --- |
| 1 | `switch`/`select` case mutation. The tagless form is already a boolean context Form C can express; the tagged form needs the case expression selected at the point it is evaluated, and a type switch's labels name types rather than values | `case-label` no longer appears in `go-mutants list` over `fixtures/discovery`, except for a type switch, and `fixtures/families` holds a live candidate in a tagless `switch`, a tagged one, and a `select` clause |
