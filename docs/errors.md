...
| `GOM4050` | A `--changed` run whose diff edited test files. A test file holds no mutant, so it narrows nothing towards itself; what a test edit moves is which mutants the suite kills, and the mapping that could name those is built from the selection this narrowing produces. The run narrows as asked and states the part it could not account for. | nothing, if the diff also edited the code under test; run without `--changed` to measure the mutants the edited tests reach |
