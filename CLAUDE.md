...
| that instrumenting once is a measurement | the counts that golden fixes | `internal/engine/overlaydoc_test.go` |
...
| that every catalogued mutant reaches one bucket | the outcome vocabulary, both ways | `internal/mutation/accounting_test.go` |
| that every `go` command of ours answered the workspace question | the recording every run keeps | `internal/engine/workceiling_integration_test.go` |
